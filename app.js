// --- CONEXÃO COM O SUPABASE ---
        ...
        // Inicializa apontando para o schema criado no SQL, sem travar o protótipo caso o CDN falhe.
        if (window.supabase && typeof window.supabase.createClient === 'function') {
            supabaseClient = window.supabase.createClient(supabaseUrl, supabaseKey, {
                db: { schema: 'gestao_atividades' }
            });
        } else {
            console.warn('Supabase indisponível no carregamento. Persistência local mantida via localStorage.');
        }
        ...
        const CLOSED_STATUSES = ['Baixada', 'Cancelada', 'Reprovada'];
        const DIRECTION_PENDING_STATUSES = ['Aberta', 'Em avaliação', 'Concluída'];
        ...
        function isDirectionPending(tarefa) {
            return tarefa && DIRECTION_PENDING_STATUSES.includes(tarefa.status);
        }

        function ensureTaskHistory(tarefa) {
            if (!Array.isArray(tarefa.historico)) tarefa.historico = [];
            return tarefa.historico;
        }

        function normalizeDatabase(data) {
            const normalized = Object.assign({}, JSON.parse(JSON.stringify(defaultData)), data || {});
            normalized.listas = Object.assign({}, defaultData.listas, normalized.listas || {});
            normalized.usuarios = Array.isArray(normalized.usuarios) ? normalized.usuarios : JSON.parse(JSON.stringify(defaultData.usuarios));
            normalized.tarefas = Array.isArray(normalized.tarefas) ? normalized.tarefas : [];
            normalized.currentUser = normalized.currentUser || normalized.usuarios[0] || JSON.parse(JSON.stringify(defaultData.currentUser));
            normalized.tarefas.forEach(tarefa => ensureTaskHistory(tarefa));
            return normalized;
        }
        ...
        function initDatabase() {
            ...
            if (saved) {
                try {
                    DB = normalizeDatabase(JSON.parse(saved));
                } catch (error) {
                    console.error('Falha ao ler dados locais. Base reiniciada com dados padrão.', error);
                    localStorage.removeItem(STORAGE_KEY);
                    DB = normalizeDatabase(JSON.parse(JSON.stringify(defaultData)));
                    if(DB.tarefas.length === 0) seedInitialData();
                    saveDatabase();
                    showToast('Dados locais corrompidos foram reiniciados com segurança.', 'error');
                }
            } else {
                DB = normalizeDatabase(JSON.parse(JSON.stringify(defaultData)));
        ...
        function saveDatabase() {
            try {
                ...
                showToast('Não foi possível salvar no navegador. Verifique espaço/permissões.', 'error');
            }
        }
            ...
            DB.tarefas = [
                {
                    ...
                    conclusao: null, baixa: null, historico: []
                },
                {
                    ...
                    baixa: null, historico: []
                },
                {
                    id: 'TK-1003', dataAbertura: new Date().toISOString().split('T')[0], prioridade: 'Baixa', status: 'Em avaliação',
                    ...
                    avaliacao: null, conclusao: null, baixa: null, historico: []
                }
            ];
        }
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className = `flex items-center p-4 rounded-lg shadow-xl min-w-[300px] toast-enter transition-all ${styles[toastType]}`;
            toast.innerHTML = `${icons[toastType]}<span class="font-medium text-sm">${safeText(message)}</span>`;
            
            container.appendChild(toast);
            refreshIcons({ root: toast });
        ...
        const formatDate = (dateStr) => {
            ...
            if(parts.length !== 3) return safeText(dateStr);
            const [y, m, d] = parts;
            return safeText(`${d}/${m}/${y}`);
        };
        ...
        function getNextActionBadge(tarefa) {
            ...
            return `<span class="inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${map[text] || 'bg-slate-50 text-slate-600 border-slate-200'}">${safeText(text)}</span>`;
        }
        ...
        function getPrazoMicrocopy(limitDate, status) {
            ...
            if(isClosedStatus(status)) return 'Sem ação pendente';
        ...
        function focarDashboard(tipo) {
            ...
            if(tipo === 'execucao') {
                if (DB.currentUser.role === 'admin') {
                    ...
                    navigate('admin');
                    const statusAdmin = document.getElementById('filter-admin-status');
                    if(statusAdmin) { statusAdmin.value = 'Em execução'; renderAdminTable(currentAdminTab); }
                } else {
                    navigate('user-tasks');
        ...
        function toggleUserRole() {
            ...
            saveDatabase();
            updateUIForRole();
            showToast(`Sessão alterada para: ${DB.currentUser?.nome || 'Usuário'}`, 'info');
            navigate(DB.currentUser.role === 'admin' ? 'dashboard' : 'user-tasks');
        }

        function updateBadges() {
            // Contadores da Sidebar e Abas
            const myTasksCount = DB.tarefas.filter(t => t.responsavelId === DB.currentUser.id && !isClosedStatus(t.status)).length;
            const adminPending = DB.tarefas.filter(t => isDirectionPending(t)).length;
        ...
        function updateUIForRole() {
            document.getElementById('user-name').innerText = DB.currentUser?.nome || 'Usuário';
            ...
            document.getElementById('user-avatar').innerText = (DB.currentUser?.nome || 'U').charAt(0);
        ...
        function navigate(viewId) {
            if (viewId === 'admin' && DB.currentUser.role !== 'admin') {
                showToast('Apenas a Direção acessa esta área.', 'info');
                viewId = 'user-tasks';
            }

            if (!['dashboard', 'user-tasks', 'admin'].includes(viewId)) {
                viewId = 'dashboard';
            }
            ...
            const viewTitle = titles[viewId] || titles.dashboard;
            document.getElementById('page-title').innerText = viewTitle.title;
            document.getElementById('page-subtitle').innerText = viewTitle.sub;
            
            const tpl = document.getElementById(`tpl-${viewId || 'dashboard'}`).content.cloneNode(true);
        ...
        function renderDashboardData() {
            ...
            document.getElementById('dash-pendentes').innerText = DB.tarefas.filter(t => isDirectionPending(t)).length;
            document.getElementById('dash-atraso').innerText = DB.tarefas.filter(t => isOverdue(t.prazoLimite) && !isClosedStatus(t.status)).length;
            ...
            if(DB.tarefas.length === 0) {
                ...
                DB.tarefas.slice(0, 6).forEach(t => {
                    ...
                    const isLate = isOverdue(t.prazoLimite) && !isClosedStatus(t.status);
                    const prazoText = getPrazoMicrocopy(t.prazoLimite, t.status);

                    tbody.innerHTML += `
                        <tr class="hover:bg-slate-50 cursor-pointer transition-colors group" onclick="abrirDetalhes(${safeJsArg(t.id)})">
                            <td class="py-3 px-4 font-bold text-slate-700" data-label="Ficha / ID">${safeText(t.id)}</td>
                            <td class="py-3 px-4 truncate max-w-[190px] font-medium text-slate-800" data-label="Resumo" title="${safeText(t.titulo)}">
                                <span class="group-hover:text-brand-600 transition-colors">${safeText(t.titulo)}</span>
                                <div class="mt-1"><span class="px-2 py-0.5 rounded-md text-[10px] font-bold border ${getPriorityColor(t.prioridade)}">${safeText(t.prioridade)}</span></div>
                            </td>
                            <td class="py-3 px-4 text-slate-500" data-label="Executor">${safeText(respName)}</td>
                            <td class="py-3 px-4" data-label="Status"><span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getStatusColor(t.status)}">${safeText(t.status)}</span></td>
                            ...
                            <td class="py-3 px-4 ${isLate ? 'text-red-600 font-bold' : 'text-slate-600'}" data-label="Prazo">
                                ...
                                <p class="text-[10px] ${isLate ? 'text-red-500' : 'text-slate-400'} mt-1">${safeText(prazoText)}</p>
                            </td>
                        </tr>
                    `;
                });
            ...
            if(ctx && window.Chart) {
                ...
                ctx.parentElement.innerHTML = '<div class="text-xs text-slate-400 text-center p-6 border border-dashed border-slate-200 rounded-xl">Gráfico indisponível: biblioteca Chart.js não carregada.</div>';
            }
            refreshIcons();
        }
        ...
        function renderUserTasks() {
            ...
            const term = searchObj ? normalizeSearch(searchObj.value) : '';
            const statusFilter = statusObj ? statusObj.value : '';
            const priorityFilter = priorityObj ? priorityObj.value : '';

            let myTasks = DB.tarefas.filter(t => t.responsavelId === DB.currentUser.id && !isClosedStatus(t.status));

            if(term) {
                myTasks = myTasks.filter(t => normalizeSearch(t.titulo).includes(term) || normalizeSearch(t.id).includes(term));
            }
                ...
                grid.innerHTML += `
                    <div class="bg-white border-2 ${borderStatus} rounded-2xl p-5 shadow-sm hover:shadow-lg transition-all relative flex flex-col group">
                        ...
                        <div class="flex justify-between items-center mb-3 gap-2">
                            <span class="text-xs font-black text-slate-400 bg-slate-100 px-2 py-1 rounded-md">${safeText(t.id)}</span>
                            <span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getStatusColor(t.status)}">${safeText(t.status)}</span>
                        </div>

                        <div class="mb-3 flex flex-wrap gap-2">
                            <span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getPriorityColor(t.prioridade)}">Prioridade ${safeText(t.prioridade)}</span>
                            ${getNextActionBadge(t)}
                        </div>

                        <h4 class="font-bold text-slate-800 text-lg leading-tight mb-2 group-hover:text-brand-600 transition-colors">${safeText(t.titulo)}</h4>
                        <p class="text-sm text-slate-500 line-clamp-2 mb-5 flex-1">${safeText(t.descricao)}</p>
                        ...
                        <div class="rounded-xl bg-slate-50 border border-slate-100 p-3 mb-4">
                            ...
                            <p class="text-xs font-medium text-slate-700 line-clamp-2">${safeText(t.resultadoEsperado)}</p>
                        </div>
                        ...
                        <div class="border-t border-slate-100 pt-4 flex justify-between items-center mt-auto">
                            <div class="flex flex-col">
                                ...
                                <span class="text-[10px] ${isLate ? 'text-red-500' : 'text-slate-400'} mt-0.5">${safeText(prazoText)}</span>
                            </div>
                            <button onclick="abrirDetalhes(${safeJsArg(t.id)})" class="bg-slate-900 hover:bg-brand-600 text-white px-4 py-2 rounded-lg text-xs font-bold transition-colors shadow-sm flex items-center">
        ...
        function renderAdminTable(filterTab) {
            ...
            if(term) list = list.filter(t => normalizeSearch(t.titulo).includes(term) || normalizeSearch(t.id).includes(term));
            if(statusFilter) list = list.filter(t => t.status === statusFilter);
            if(priorityFilter) list = list.filter(t => t.prioridade === priorityFilter);
            if(prazoFilter === 'vencidas') list = list.filter(t => isOverdue(t.prazoLimite) && !isClosedStatus(t.status));
            if(prazoFilter === 'em-dia') list = list.filter(t => !isOverdue(t.prazoLimite) || isClosedStatus(t.status));
                ...
                tbody.innerHTML += `
                    <tr class="hover:bg-slate-50/80 transition-colors border-b border-slate-100 group">
                        <td class="py-4 px-6" data-label="Ficha e Título">
                            <p class="font-black text-slate-800 text-sm mb-0.5 group-hover:text-brand-600 transition-colors">${safeText(t.id)}</p>
                            <p class="text-xs text-slate-500 truncate max-w-[250px]" title="${safeText(t.titulo)}">${safeText(t.titulo)}</p>
                            <span class="inline-block mt-2 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border ${getPriorityColor(t.prioridade)}">${safeText(t.prioridade)}</span>
                        </td>
                        <td class="py-4 px-6" data-label="Setores">
                            <p class="text-xs text-slate-700 font-semibold flex items-center mb-0.5" title="Solicitante"><i data-lucide="arrow-up-right" class="w-3 h-3 mr-1 text-slate-400"></i> ${safeText(t.setorSolicitante)}</p>
                            <p class="text-xs text-slate-500 flex items-center" title="Executor"><i data-lucide="user" class="w-3 h-3 mr-1 text-brand-400"></i> ${safeText(respName)}</p>
                        </td>
                        <td class="py-4 px-6" data-label="Status"><span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getStatusColor(t.status)} shadow-sm">${safeText(t.status)}</span></td>
                        ...
                        <td class="py-4 px-6 text-xs" data-label="Prazos">
                            <div class="flex flex-col gap-1">
                                ...
                                <span class="${isLate ? 'text-red-500 font-bold' : 'text-slate-400'}">${safeText(prazoText)}</span>
                            </div>
                        </td>
                        <td class="py-4 px-6 text-right" data-label="Ação">
                            <button onclick="abrirDetalhes(${safeJsArg(t.id)})" class="px-4 py-2 bg-white border border-slate-300 text-slate-700 text-xs font-bold rounded-lg hover:bg-brand-50 hover:text-brand-600 hover:border-brand-300 transition-all shadow-sm">
        ...
        function abrirDetalhes(id) {
            ...
            const prazoText = getPrazoMicrocopy(t.prazoLimite, t.status);
            const custoTexto = t.geraCusto === 'Sim' ? `${safeText(t.tipoCusto)} (R$ ${safeText(t.valorEstimado)})` : 'Sem Custo';
            const vinculoCustoTexto = `${safeText(t.obraVinculada)} ${t.cliente ? `(${safeText(t.cliente)})` : ''} <br> ${custoTexto}`;
                    ...
                    <div class="relative z-10 flex items-center gap-4">
                        ...
                        <div>
                            <div class="flex items-center gap-3 mb-1">
                                <span class="text-xs font-black text-slate-400 tracking-widest bg-slate-800 px-2 py-0.5 rounded">${safeText(t.id)}</span>
                                <span class="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase border ${getStatusColor(t.status)} shadow-sm">${safeText(t.status)}</span>
                            </div>
                            <h2 class="text-xl font-bold leading-tight">${safeText(t.titulo)}</h2>
                            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-white/10 text-slate-200 border border-white/10">
                                    <i data-lucide="route" class="w-3 h-3 mr-1"></i> Próximo passo: <strong class="ml-1 text-white">${safeText(nextActionText)}</strong>
                                </span>
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full ${isLate ? 'bg-red-500/20 text-red-100 border-red-400/30' : 'bg-white/10 text-slate-200 border-white/10'} border">
                                    <i data-lucide="calendar-clock" class="w-3 h-3 mr-1"></i> ${safeText(prazoText)}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto bg-slate-50 flex flex-col lg:flex-row custom-scrollbar">
                    ...
                    <div class="w-full lg:w-[55%] p-6 lg:p-8 space-y-8 border-r border-slate-200 bg-white">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                                ...
                                <div><span class="text-slate-500 block text-[10px] uppercase font-bold">Prioridade</span><span class="font-medium text-slate-800">${safeText(t.prioridade)}</span></div>
                                ...
                                <div><span class="text-slate-500 block text-[10px] uppercase font-bold">Solicitante</span><span class="font-medium text-slate-800">${safeText(t.setorSolicitante)}</span></div>
                                <div class="sm:col-span-2"><span class="text-slate-500 block text-[10px] uppercase font-bold">Responsável</span><span class="font-medium text-slate-800">${safeText(t.setorResponsavel)} / ${safeText(respName)}</span></div>
                                ...
                                <div><span class="text-slate-500 block text-[10px] uppercase font-bold">Vínculo / Custos</span><span class="font-medium text-slate-800 text-xs">${vinculoCustoTexto}</span></div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            ${t.obsInicial ? `<div class="mb-4 bg-blue-50 border border-blue-100 p-3 rounded-lg"><span class="text-blue-600 block text-[10px] uppercase font-bold mb-1">Contexto Inicial</span><span class="italic text-slate-700 text-sm">"${safeText(t.obsInicial)}"</span></div>` : ''}
                            <p class="text-sm text-slate-700 bg-white border border-slate-200 p-4 rounded-xl leading-relaxed whitespace-pre-wrap">${safeText(t.descricao)}</p>
                            ...
                            <div class="mt-4 bg-emerald-50 border border-emerald-100 p-4 rounded-xl flex items-start">
                                ...
                                <div>
                                    ...
                                    <span class="text-sm font-medium text-emerald-900">${safeText(t.resultadoEsperado)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full lg:w-[45%] p-6 lg:p-8 space-y-6 relative">
                        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                            ...
                            <h3 class="text-lg font-black text-slate-800">${safeText(nextActionText)}</h3>
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-3">
                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    ...
                                    <span class="inline-flex px-2.5 py-1 rounded-md text-[10px] font-bold uppercase border ${getStatusColor(t.status)}">${safeText(t.status)}</span>
                                </div>
                                <div class="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                    ...
                                    <p class="text-[10px] ${isLate ? 'text-red-500' : 'text-slate-400'} mt-0.5">${safeText(prazoText)}</p>
                                </div>
                            </div>
                        </div>
            `;
                ...
                html += `
                    <div class="relative pl-6 pb-6">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm text-sm">
                            <p class="mb-1"><span class="font-bold text-slate-700">${safeText(t.avaliacao.encaminhamento)}</span> por ${safeText(t.avaliacao.aprovador)}</p>
                            <p class="text-xs text-slate-500 mb-2"><i data-lucide="calendar" class="inline w-3 h-3 mr-1"></i>${formatDate(t.avaliacao.data)}</p>
                            ${t.avaliacao.observacoes ? `<div class="bg-slate-50 p-2 rounded border border-slate-100 italic text-slate-600 text-xs">"${safeText(t.avaliacao.observacoes)}"</div>` : ''}
                        </div>
                    </div>`;
            } else if (isAdmin && (t.status === 'Aberta' || t.status === 'Em avaliação')) {
                html += `
                    <div class="relative pl-6 pb-6">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-xl p-5 shadow-sm">
                            ...
                            <button onclick="executarAcao(${safeJsArg(t.id)}, 'avaliar')" class="w-full py-2.5 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-md flex items-center justify-center">
            ...
            if (t.conclusao) {
                html += `
                    <div class="relative pl-6 pb-6">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm text-sm">
                            <p class="mb-1"><span class="font-bold text-slate-700">Entregue</span> por ${safeText(t.conclusao.informante)}</p>
                            <p class="text-xs text-slate-500 mb-3"><i data-lucide="calendar" class="inline w-3 h-3 mr-1"></i>${formatDate(t.conclusao.data)}</p>
                            <div class="bg-indigo-50 p-3 rounded-lg border border-indigo-100 text-slate-700 text-sm whitespace-pre-wrap"><strong>Relatório:</strong><br>${safeText(t.conclusao.resumo)}</div>
                        </div>
                    </div>`;
                    ...
                    html += `
                        <div class="relative pl-6 pb-6">
                            ...
                            <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center shadow-sm">
                                ...
                                <button onclick="executarAcao(${safeJsArg(t.id)}, 'iniciar')" class="px-6 py-2.5 bg-amber-500 text-white text-sm font-bold rounded-lg hover:bg-amber-600 shadow-md w-full transition-all hover:scale-[1.02]">
                    ...
                    html += `
                        <div class="relative pl-6 pb-6">
                            ...
                            <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-5 shadow-sm">
                                ...
                                <button onclick="executarAcao(${safeJsArg(t.id)}, 'concluir')" class="w-full py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 transition-colors shadow-md flex items-center justify-center">
            ...
            if (Array.isArray(t.historico) && t.historico.length > 0) {
                html += `
                    <div class="relative pl-6 pb-6">
                        <div class="absolute left-0 top-1 w-3.5 h-3.5 rounded-full bg-orange-500 ring-4 ring-white"></div>
                        <h4 class="text-xs font-bold text-orange-600 uppercase tracking-widest mb-2">Histórico de Devoluções</h4>
                        <div class="space-y-3">
                            ${t.historico.map(item => `
                                <div class="bg-orange-50 border border-orange-100 rounded-xl p-3 text-xs text-slate-700">
                                    <div class="flex flex-wrap justify-between gap-2 mb-2">
                                        <span class="font-bold text-orange-700">${safeText(item.tipo || 'Devolução')}</span>
                                        <span class="text-slate-500">${formatDate(item.data)}</span>
                                    </div>
                                    <p><span class="font-bold">Responsável:</span> ${safeText(item.responsavel || '--')}</p>
                                    <p class="mt-1 whitespace-pre-wrap"><span class="font-bold">Parecer:</span> ${safeText(item.parecer || '')}</p>
                                    ${item.conclusaoAnterior?.resumo ? `<div class="mt-2 border-t border-orange-100 pt-2 text-slate-500 whitespace-pre-wrap"><span class="font-bold">Conclusão devolvida:</span><br>${safeText(item.conclusaoAnterior.resumo)}</div>` : ''}
                                </div>
                            `).join('')}
                        </div>
                    </div>`;
            ...
            if (t.baixa) {
                html += `
                    <div class="relative pl-6 pb-2">
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm text-sm">
                            <p class="mb-1"><span class="font-bold text-slate-700">Baixa Aprovada</span> por ${safeText(t.baixa.responsavel)}</p>
                            <p class="text-xs text-slate-500 mb-3"><i data-lucide="calendar" class="inline w-3 h-3 mr-1"></i>${formatDate(t.baixa.data)}</p>
                            <div class="bg-green-50 p-3 rounded-lg border border-green-100 italic text-slate-700 text-sm">"${safeText(t.baixa.parecer)}"</div>
                    ...
                    <div class="relative pl-6 pb-2">
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-xl p-5 shadow-sm">
                            ...
                            <div class="flex gap-3">
                                <button onclick="executarAcao(${safeJsArg(t.id)}, 'reabrir')" class="w-1/3 py-2.5 bg-white border border-slate-300 text-slate-700 text-xs font-bold rounded-lg hover:bg-red-50 hover:text-red-600 hover:border-red-300 transition-all flex flex-col items-center justify-center">
                                    <i data-lucide="corner-up-left" class="w-4 h-4 mb-1"></i> Devolver
                                </button>
                                <button onclick="executarAcao(${safeJsArg(t.id)}, 'baixar')" class="w-2/3 py-2.5 bg-green-600 text-white text-sm font-bold rounded-lg hover:bg-green-700 transition-all shadow-md flex items-center justify-center">
        ...
        function executarAcao(id, acao) {
            ...
            if (['avaliar', 'baixar', 'reabrir'].includes(acao) && !isAdmin) {
                showToast('Apenas a Direção pode executar esta ação.', 'error');
                return;
            }

            if (['iniciar', 'concluir'].includes(acao) && !isOwner) {
                showToast('Somente o responsável direto pode executar esta ação.', 'error');
                return;
            }

            if (acao === 'avaliar') {
                if (!(t.status === 'Aberta' || t.status === 'Em avaliação')) {
                    showToast('Esta ficha não está pendente de avaliação.', 'info');
                    return;
                }
                ...
                t.avaliacao = { data: hoje, aprovador: DB.currentUser.nome, encaminhamento: encam, observacoes: document.getElementById('act-obs-direcao').value.trim() };
                t.status = encam === 'Reprovada' ? 'Reprovada' : 'Aprovada';
                showToast(`Ficha ${encam.toLowerCase()}!`, encam === 'Reprovada' ? 'error' : 'success');
            } 
            else if (acao === 'iniciar') {
                if (t.status !== 'Aprovada') {
                    showToast('Esta ficha não está liberada para início.', 'info');
                    return;
                }
                ...
                if (t.status !== 'Em execução') {
                    showToast('Esta ficha não está em execução.', 'info');
                    return;
                }

                const res = document.getElementById('act-resumo').value.trim();
                ...
                if (t.status !== 'Concluída') {
                    showToast('Esta ficha não está pronta para baixa.', 'info');
                    return;
                }

                const par = document.getElementById('act-parecer').value.trim();
                ...
                if (t.status !== 'Concluída') {
                    showToast('Somente fichas concluídas podem ser devolvidas ao executor.', 'info');
                    return;
                }
                ...
                ensureTaskHistory(t).push({
                    tipo: 'Devolução',
                    data: hoje,
                    responsavel: DB.currentUser.nome,
                    parecer: par,
                    conclusaoAnterior: t.conclusao ? Object.assign({}, t.conclusao) : null
                });

                t.conclusao = null; // Remove a conclusão ativa, preservando a anterior no histórico
                t.status = 'Em execução'; // Volta para o executor responsável
            ...
            const activeViewButton = document.querySelector('aside nav button.bg-brand-600');
            const view = activeViewButton ? activeViewButton.id.replace('nav-', '') : 'dashboard';
            navigate(view);
            if(view === 'admin') {
